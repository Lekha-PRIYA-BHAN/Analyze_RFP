...
sustainability = """As an expert in Microsoft Cloud for Sustainability, Power Apps, and DataVerse answer the following query as per the given context:
context:
-----
{context}
-----

Use the above context to provide an answer for the following query which will be around Microsoft Cloud for Sustainability.
...
template['sustainability'] = sustainability
...
st.set_page_config(page_title='Know Your Cloud Architecture and Automation', layout='wide')
...
with st.sidebar.expander(" 🛠️ Settings ", expanded=False):
    ...
    TEMPLATE = st.selectbox(label='Template', options=['sustainability', 'yamlsearch', 'terraform', 'normal'])
...
if user_input and st.sidebar.button("Execute Query", type='primary'):
    context_docs = knowledge_base.similarity_search(user_input, k=6, fetch_k=6)
    print("#######################START#########################################")
    print(prompt)
    print("#########################END#########################################")
    source_set = set({})
